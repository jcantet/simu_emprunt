...
ui <- dashboardPage(
    ...
    dashboardSidebar(width = 220,
                     sidebarMenu(tags$script(HTML("$('body').addClass('fixed');")), # Barre latérale fixée
                                 ...
                                 numericInput("taux","Taux d'intérêt (%)", min = 0, max = 5, value = 1.05, step = 0.1),
                                 numericInput("assurance","Assurance emprunteur (%)", min = 0, max = 5, step = 0.1, value = 0.34),
                                 sliderInput("duree","Durée du prêt (an)", min = 5, max = 30, step = 1, value = 20, post = " ans")
                     )
    ),
